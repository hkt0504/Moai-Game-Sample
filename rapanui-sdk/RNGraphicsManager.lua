...
function RNGraphicsManager:allocateDeck2DGfx(path)
    ...
    object.image = MOAIImageTexture.new()
    object.image:load(path, MOAIImage.TRUECOLOR + MOAIImage.PREMULTIPLY_ALPHA) 
    ...
    object.deck = MOAISpriteDeck2D.new()
    ...
    return object.deck, object.image
end
...
function RNGraphicsManager:getNumGfx()
  return table.getn(self.gfx)
end

--MANUALLY ADDED METHOD!!!
function RNGraphicsManager:deallocateGfxWithIdx(indexToRemove, garbagecollect)
    if indexToRemove ~= nil and indexToRemove < table.getn(self.gfx) then
        local object = self.gfx[indexToRemove]
        for i = indexToRemove, #self.gfx - 1 do
            self.gfx[i] = self.gfx[i + 1]
        end
...
function RNGraphicsManager:deallocateAllGfx()
  for i = 1, table.getn(self.gfx) do
...
function RNGraphicsManager:allocateTileDeck2DGfx(path, sx, sy, gutter)
    ...
    object.deck:setSize(px, py, 1 / gpx, 1 / gpy, 0, 0, 1 / px, 1 / py)
...
function RNGraphicsManager:allocateFont(path)
    ...
    --[[ print("Creating dynamic load font for '" .. path .. "'")
    object.font:setReader(MOAIFreeTypeFontReader.new())
    object.font:setCache(MOAIGlyphCache.new()) --]]
    object.isInAtlas = false
