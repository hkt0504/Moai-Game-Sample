...
local function fieldAccessListener(self, key)
    ...
    if object.isPhysical == false then
        ...
        if key ~= nil and key == "rotation" then
            _, _, object.rotation = object:getProp():getRot()
        end
    end
...
function RNObject:initWithImage2(image)
    ...
    if type(image) == "string" then
        if RNGraphicsManager:getAlreadyAllocated(image) then
            deck, numberInAtlas, moaiImage = RNGraphicsManager:getDeckByPath(image)
        else
            deck, moaiImage = RNGraphicsManager:allocateDeck2DGfx(image)
        end
    end
...
function RNObject:initWithAnim2(image, sx, sy, scaleX, scaleY, gutter)
    ...
    if type(image) == "string" then
        if RNGraphicsManager:getAlreadyAllocated(image) then
            ...
            deck = RNGraphicsManager:allocateTileDeck2DGfx(image, sx, sy, gutter)
        end
    else
    ...
    if type(image) == "string" then self.name = image else self.name = "" end
    self.originalWidth = sx
    self.originalHeight = sy
...
function RNObject:play(sequenceName, speed, repeatTimes, onStop)
    ...
    if rightSequenceToPlay ~= nil then
      local rightSequence = self.sequenceList[rightSequenceToPlay]
      --set sequence values
...
function RNObject:setScaleX(value)
    ...
    self.originalWidth = self.originalWidth * (value / xs)
end

function RNObject:setScaleY(value)
    local xs, ys = self.prop:getScl()
    self.prop:setScl(xs, value)
    self.originalHeight = self.originalHeight * (value / ys)
end
...
-- MODIFIED!!, setting self.visible value
...
-- CHANGED!!! taking abs of originalwidth and height
function RNObject:isInRange(x, y)
    --print("CHECKING! " .. self.name .. " :" .. x .. " " .. y)
    
    if self.root ~= nil then
      local actualX = self.x
      local actualY = self.y
      local rootX, rootY = self.root:getLoc()
      local xs, ys = self.prop:getScl()
      actualX = actualX + rootX - self.pivotx
      actualY = (actualY  - math.abs(self.originalHeight) * ys) + rootY - self.pivoty
      
      --[[print("x: " .. x)
      print("y: " .. y)
      print("self.pivotx " .. self.pivotx)
      print("self.pivoty " .. self.pivoty)
      print("math.abs(self.originalWidth): " .. math.abs(self.originalWidth))
      print("math.abs(self.originalHeight): " .. math.abs(self.originalHeight))
      print("self.x: " .. self.x)
      print("self.y: " .. self.y)
      print("scaleX " .. xs)
      print("scaleY " .. ys)
      ...
      if self.visible
        and x >= actualX
        and x <= actualX + (math.abs(self.originalWidth) * xs)
        and y >= actualY
        and y <= actualY + (math.abs(self.originalHeight) * ys)
      then
          --print("IS INSIDE!!!!")
          return true
      end
      ...
      buttonx = x + math.abs(self.originalWidth) / 2
      buttony = y + math.abs(self.originalHeight) / 2
      if self.visible
        and buttonx >= self.x
        and buttonx <= self.x + math.abs(self.originalWidth)
        and buttony >= self.y
        and buttony <= self.y + math.abs(self.originalHeight)
      then
          return true
      end
    end
