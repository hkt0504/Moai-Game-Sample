...
function RNPhysics.createBodyFromImage(image, ...)
    ...
    --if there are
    if (arg ~= nil and table.getn(arg) ~= nil and table.getn(arg) > 0) then
    ...
    if (typeGiven == true) then fixturesReceived = table.getn(arg) - 1 else fixturesReceived = table.getn(arg)
    end
  ...
  the image. --]]
    ...
    if (fixturesReceived ~= 0 and firstFixtureArgument ~= nil) then
        ...
        --specified as first optional argument so the others until table.getn(arg) are all fixture/tables)
        for i = firstFixtureArgument, table.getn(arg), 1 do
    ...
    else --if (fixturesReceived == 0) then --else arg[1]~=nil
