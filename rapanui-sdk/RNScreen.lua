...
function RNScreen:initWith(width, height, screenWidth, screenHeight)
    ...
    --self.viewport:setSize(MOAIEnvironment.horizontalResolution, MOAIEnvironment.verticalResolution)
    --self.viewport:setScale(MOAIEnvironment.horizontalResolution, -MOAIEnvironment.verticalResolution)
    
    self.layer:pushRenderPass()
end
...
function RNScreen:addRNObject(object, mode, layer)
    ...
    object:getProp():setPartition(layer)
...
function RNScreen:getObjectWithHighestLevelOn(x, y)
    ...
    if config.stretch.status == true then
        if config.stretch.letterbox == true then
            ...
            props = { self.mainPartition:hullListForPoint(toGetX, toGetY + RNFactory.statusBarHeight * y / RNFactory.height, 0, MOAILayer.SORT_PRIORITY_DESCENDING) }
        else
            local toGetX, toGetY = (x - RNFactory.ofx) * RNFactory.Ax, (y - RNFactory.ofy) * RNFactory.Ay
            props = { self.mainPartition:hullListForPoint(toGetX, toGetY + RNFactory.statusBarHeight * y / RNFactory.height, 0, MOAILayer.SORT_PRIORITY_DESCENDING) }
        end
    else
        props = { self.mainPartition:hullListForPoint(x, y + RNFactory.statusBarHeight * y / RNFactory.height, 0, MOAILayer.SORT_PRIORITY_DESCENDING) }
    end

    for i = 1, #props do
        local currentProp = props[i]
        if currentProp.RNObject ~= nil and currentProp.RNObject.touchable == true then
